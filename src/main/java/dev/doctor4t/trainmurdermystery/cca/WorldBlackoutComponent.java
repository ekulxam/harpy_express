...
public class WorldBlackoutComponent implements AutoSyncedComponent, ServerTickingComponent {
    ...
    public void reset() {
        for (BlackoutDetails detail : this.blackouts) {
            detail.end(this.world); // I read this as detail.endTheWorld lol - SkyNotTheLimit
        }
    ...
    public boolean triggerBlackout() {
        Box area = GameConstants.PLAY_AREA;
        if (this.ticks > 0) {
            return false;
        }

        BlockPos.stream((int) area.minX, (int) area.minY, (int) area.minZ, (int) area.maxX, (int) area.maxY, (int) area.maxZ).forEach(blockPos -> {
            ...
            BlackoutDetails detail = new BlackoutDetails(blockPos, duration, state.get(Properties.LIT));
